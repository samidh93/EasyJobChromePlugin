(() => {
  // src/LinkedInJobHelper.js
  var LinkedInJobHelper = class {
  ...
  var LinkedInJobHelper_default = LinkedInJobHelper;

  // src/content.js
  window.onload = async () => {
    const searchElement = document.querySelector(".scaffold-layout.jobs-search-two-pane__layout");
    if (searchElement) {
      const totalJobs = LinkedInJobHelper_default.getTotalJobsSearchCount(searchElement);
      console.log(`Found ${totalJobs} jobs.`);
      const pagesAvailable = LinkedInJobHelper_default.getAvailablePages(searchElement);
      console.log(`Available pages: ${pagesAvailable}`);
      const jobs = await LinkedInJobHelper_default.getListOfJobsOnPage(searchElement);
      console.log("Jobs on page:", jobs);
      for (const job of jobs) {
        if (LinkedInJobHelper_default.isJobApplied(job)) {
          continue;
        }
      }
